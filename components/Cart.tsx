...
const Cart: React.FC<CartProps> = ({ items, isOpen, onClose, onRemove, onUpdateQty, onClear }) => {
      ...
      <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-slideInRight">
        ...
        <div className="p-8 border-b border-slate-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 text-sm font-bold mt-1">共 {totalQty} 個品項</p>
          </div>

          <button
            onClick={onClose}
            className="w-12 h-12 rounded-full border border-slate-100 text-slate-400 hover:bg-slate-50 transition-colors flex items-center justify-center"
        ...
        <div className="flex-grow overflow-y-auto p-8 space-y-6">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center">
              ...
              <button
                onClick={onClose}
                className="bg-emerald-600 text-white px-8 py-3 rounded-2xl font-black shadow-lg shadow-emerald-100"
              >
                前往挑選
              </button>
            </div>
              ...
              {items.map((item) => (
                <div key={item.id} className="flex gap-5 group">
                  <div className="w-20 h-20 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-300 relative border border-slate-100">
                    <i
                      className={`fa-solid ${
                        item.type === 'mealkit' ? 'fa-kitchen-set' : 'fa-basket-shopping'
                      } text-2xl`}
                    ></i>
                  </div>
                    ...
                    <div className="flex justify-between items-end">
                      <div className="flex items-center bg-slate-50 rounded-xl p-1 border border-slate-100">
                        <button
                          ...
                          className="w-8 h-8 rounded-lg hover:bg-white hover:shadow-sm text-slate-500 transition-all font-black"
