...
import { parseMealKits, parseProducts } from './constants';
import { CartItem, MealKit, Product, StoreTab } from './types';
...
const App = () => {
  ...
  const [storeTab, setStoreTab] = useState<StoreTab>('fruit');
  const [cart, setCart] = useState<CartItem[]>([]);
  const [isCartOpen, setIsCartOpen] = useState(false);

  const cartCount = useMemo(() => cart.reduce((s, i) => s + i.quantity, 0), [cart]);

  const addToCart = (id: string, name: string, price: number, type: CartItem['type']) => {
    setCart((prev) => {
  ...
  const custardApple = useMemo(() => products.find((p) => p.id === 'F-C-001'), [products]);
      ...
      <section className="py-12 px-4 max-w-7xl mx-auto">
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {[
            ...
            <button
              key={t.id}
              onClick={() => setStoreTab(t.id as StoreTab)}
              className={`px-8 py-4 rounded-2xl font-black transition-all flex items-center gap-3 ${
                storeTab === t.id ? 'bg-slate-900 text-white shadow-xl scale-105' : 'bg-white text-slate-400 hover:bg-slate-50'
              }`}
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
          {storeTab === 'mealkit'
            ? mealKits.map((kit: MealKit) => (
                <MealKitCard
                  key={kit.id}
                  kit={kit}
                  onAdd={(k) => addToCart(k.id, k.name, k.price, 'mealkit')}
                />
              ))
            : filteredProducts.map((p: Product) => (
                <LocalProductCard
                  key={p.id}
                  product={p}
                  onAdd={(pp) => addToCart(pp.id, pp.name, pp.price, 'product')}
                />
              ))}
        </div>
      </section>
